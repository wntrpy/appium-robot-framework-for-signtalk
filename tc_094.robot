...
*** Test Cases ***
Send Multiple Feedback Entries
    [Documentation]    Login → Home → Profile → Settings → Feedback → Submit 3 feedbacks → PASS
    ...
    Input Text                       ${FEEDBACK_INPUT}            Great app
    Sleep                            1s
    Click Element                    ${SUBMIT_BTN}
    Sleep                            1s
    Log                              First feedback submitted.
    ...
    Wait Until Element Is Visible    ${FEEDBACK_INPUT}             ${TIMEOUT}
    Click Element                    ${FEEDBACK_INPUT}
    Input Text                       ${FEEDBACK_INPUT}             Nice App
    Sleep                            1s
    Click Element                    ${SUBMIT_BTN}
    Sleep                            1s
    Log                              Second feedback submitted.
    ...
    Wait Until Element Is Visible    ${FEEDBACK_INPUT}            ${TIMEOUT}
    Click Element                    ${FEEDBACK_INPUT}
    Input Text                       ${FEEDBACK_INPUT}            Translations are smooth.
    Sleep                            1s
    Click Element                    ${SUBMIT_BTN}
    Sleep                            1s
    Log                              Third feedback submitted.


    # --- Final assertion ---
    Log    All 3 feedback messages submitted successfully. TEST PASSED.
